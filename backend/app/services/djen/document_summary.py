...
def _document_result(excerpt: str | None) -> str | None:
    ...
    if (
        ...
        or "procedencia parcial" in folded
        or "parcial procedencia" in folded
    ):
    ...
    if (
        ...
        or "improcedencia" in folded
        or "improcedentes os pedidos" in folded
        or "improcedencia dos pedidos" in folded
    ):
    ...
    if (
        ...
        or "procedencia dos pedidos" in folded
        or "julgo procedentes" in folded
    ):
        return "Procedência"
...
def build_relevant_documents(
    ...
    with_merit_result = [
        doc
        for doc in documents
        if doc.get("resultado_documental") in merit_results
    ]

    with_any_result = [
        doc
        for doc in documents
        if doc.get("resultado_documental")
    ]

    sentence_or_judgment = [
        doc
        for doc in documents
        if any(
            token in _fold(doc.get("tipo_documento"))
            for token in ("sentenca", "acordao")
        )
    ]

    # Documento principal não é simplesmente a comunicação mais recente.
    # Priorizamos: valor judicial verificado -> decisão de mérito classificada
    # -> qualquer resultado classificado -> sentença/acórdão -> último documento.
    # Assim uma decisão procedimental posterior não esconde a sentença de mérito.
    principal = (
        with_value[-1]
        if with_value
        else (
            with_merit_result[-1]
            if with_merit_result
            else (
                with_any_result[-1]
                if with_any_result
                else (
                    sentence_or_judgment[-1]
                    if sentence_or_judgment
