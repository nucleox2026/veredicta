...
# Somente valores indenizatórios com evidência forte no dispositivo são
# tratados como valores judiciais verificados. Valores mencionados na
# narrativa dos fatos (empréstimos, boletos, pedidos, precedentes etc.) ficam
# disponíveis apenas como candidatos diagnósticos e nunca alimentam a
# jurimetria ou o snapshot monetário oficial.
VERIFIED_AWARD_CATEGORIES = {
    "dano_moral",
    "dano_estetico",
    "dano_material",
    "honorarios",
...
def extract_document_values(item: dict[str, Any]) -> dict[str, Any]:
    ...
    unverified: list[ValueEvidence] = []

    for category, (_, evidence) in selected.items():
        is_verified_award = (
            category in VERIFIED_AWARD_CATEGORIES
            and evidence.secao == "dispositivo"
            and evidence.confianca == "alta"
        )

        if is_verified_award:
            values[category] = evidence.valor_centavos
            evidences.append(asdict(evidence))
            continue

        # Valor da causa e qualquer quantia encontrada fora do dispositivo
        # continuam úteis para diagnóstico, mas não são indenização verificada.
        unverified.append(evidence)

    evidences.sort(key=lambda ev: ev["categoria"])

    candidates = discarded + unverified
    candidates.sort(
        key=lambda ev: (
            ev.data_documento or "",
            ev.categoria,
            ev.valor_centavos,
        )
    )
    ...
    return {
        ...
        "candidatos_nao_verificados": [asdict(ev) for ev in candidates],
    }
...
def extract_process_awards(
    ...
    for item in communications or []:
        ...
        for candidate in extracted.get("candidatos_nao_verificados") or []:
            ...
            enriched["tipo_documento_efetivo"] = _effective_document_type(item)
            unverified_candidates.append(enriched)
    ...
    return {
        ...
        "candidatos_nao_verificados": unverified_candidates,
    }
